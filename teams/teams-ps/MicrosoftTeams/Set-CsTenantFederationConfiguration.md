...
When set to False (the default value), all users within the tenant with `EnableFederationAccess` set to False in their  `ExternalAccessPolicy` can be added to group chats with external users only if invited by a user within the same tenant with EnableFederationAccess set to True. This users to join group chats with external users they would not normally be allow to interact with
When set of True, every user within the tenant with EnableFederationAccess set to False in the users effective External Access Policy will be blocked from being added to a group chat with an external user.
